            ...
            {steps.map((step, index) => (
              <div key={index} className="flex items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
                  index <= currentStep ? 'bg-cyan text-black' : 'bg-zinc-700 text-zinc-400'
                }`}>
                  {index < currentStep ? <CheckCircle className="w-4 h-4" /> : index + 1}
                </div>
                {index < steps.length - 1 && (
                  <div className={`w-16 h-1 mx-2 ${
                    index < currentStep ? 'bg-cyan' : 'bg-zinc-700'
                  }`} />
                )}
              </div>
            ))}
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8 mb-8">
          ...
          <div className="flex justify-between mt-8">
            ...
            {currentStep < steps.length - 1 ? (
              <button
                onClick={handleNext}
                className="px-6 py-3 bg-cyan text-black font-medium rounded-lg hover:bg-cyan-dark transition-colors"
              >
                Next
              </button>
            ) : (
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-lg text-white focus:border-cyan focus:outline-none h-24"
    ...
    <div className="space-y-8">
      ...
      <div className="p-6 bg-gradient-to-r from-cyan/20 to-cyan/5 border border-cyan/30 rounded-xl">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button className="px-6 py-3 bg-cyan text-black font-medium rounded-lg hover:bg-cyan-dark transition-colors flex items-center gap-2">
