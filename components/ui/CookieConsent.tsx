    ...
    <AnimatePresence>
      {showBanner && (
        <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="fixed bottom-4 left-4 right-4 z-[999]">
          <div className="max-w-7xl mx-auto bg-card-dark backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col md:flex-row gap-6 items-center justify-between">
            ...
            <div className="flex gap-3">
              <button onClick={() => { saveConsent({ necessary: true, analytics: false, marketing: false, functional: false }); setShowBanner(false); }} className="px-4 py-2 rounded-xl text-sm font-medium text-white bg-card-dark border border-white/10">Reject All</button>
