...
const DesktopDetailCard = ({ step, index }: { step: typeof steps[0], index: number }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: isRight ? 20 : -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className={cn("absolute top-1/2 -translate-y-1/2 hidden md:flex flex-col p-6 rounded-2xl w-80 md:w-96 bg-zinc-900/80 backdrop-blur-md border border-white/10", isRight ? "left-[calc(50%+8rem)] text-left" : "right-[calc(50%+8rem)] text-right items-end")}>
...
export function IsometricProcess() {
  return (
    <section className="w-full bg-[#050505] py-24 relative overflow-hidden flex items-center justify-center">
