...
function walk(dir) {
  return fs.readdirSync(dir).flatMap((n) => {
    ...
    if (st.isDirectory()) {
      if (n === "img" || n === "node_modules" || n.startsWith(".git"))
        return [];
      return walk(f);
    }
    return FILE_GLOBS.some((g) => g.test(n)) ? [f] : [];
  });
}
...
const mapping = [
  {
    key: "blog",
    pattern: /<img([^>]*?)src="img\/blog\/head_(\d)\.png"([^>]*)>/g,
    build: (num, pre, post) =>
      buildPicture(
        "blog",
        `head_${num}`,
  ...
  {
    key: "works",
    pattern: /<img([^>]*?)src="img\/works\/Office_(\d)\.png"([^>]*)>/g,
    build: (num, pre, post) =>
      buildPicture("works", `Office_${num}`, "png", pre, post, [320, 640, 960]),
  },
  {
    key: "faces",
    pattern: /<img([^>]*?)src="img\/faces\/Dissolve_(\d)\.png"([^>]*)>/g,
    build: (num, pre, post) =>
      buildPicture("faces", `Dissolve_${num}`, "png", pre, post, [320, 640]),
  },
];
...
function buildPicture(folder, base, ext, preAttr, postAttr, sizes) {
  ...
  const srcsetFallback = sizes
    .map(
      (w) => `img/optimized/${folder}/${base}-${Math.min(w, 1280)}.${ext} ${w}w`
    )
    .join(", ");
  const srcsetWebp = sizes
    .map(
      (w) => `img/optimized/${folder}/${base}-${Math.min(w, 1280)}.webp ${w}w`
    )
    .join(", ");
  const srcsetAvif = sizes
    .map(
      (w) => `img/optimized/${folder}/${base}-${Math.min(w, 1280)}.avif ${w}w`
    )
    .join(", ");
...
files.forEach((f) => {
  ...
  let orig = text;
  mapping.forEach((m) => {
    text = text.replace(m.pattern, (all, pre, num, post) =>
      m.build(num, pre, post)
    );
  });
  if (text !== orig) {
