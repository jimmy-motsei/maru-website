...
function listImages() {
  return glob.sync('img/**/*.{png,jpg,jpeg,gif}', { nodir: true, ignore: ['img/optimized/**'] });
}
...
async function processImage(file) {
  ...
  for (const size of SIZES) {
    ...
    for (const fmt of FORMATS) {
      ...
      if (!need) continue;
      await sharp(file).resize(size).toFormat(fmt, { quality: 70 }).toFile(outPath);
      console.log('Generated', path.relative(process.cwd(), outPath));
    }
    // Largest size keep original format (converted only if not present) for fallback at final size step
    if (size === Math.min(width, Math.max(...SIZES.filter(s => s <= width)))) {
...
(async () => {
  ...
  for (const f of files) {
    try { await processImage(f); } catch (e) { console.error('Error', f, e.message); }
  }
